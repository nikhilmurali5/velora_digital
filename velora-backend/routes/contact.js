      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#140a2c;border-radius:0 0 12px 12px;padding:20px 40px;text-align:center;border-top:1px solid rgba(168,145,212,0.12);">
            ...
            <p style="margin:0;font-size:11px;color:${C.muted};">Elevating Brands Digitally - veloradigital07@gmail.com</p>
          </td>
        </tr>
      </table>
    </td></tr>
...
/* ────────────────────────────────────────────
   Helper: send one email with full logging.
   Returns true on success, false on failure.
   Never throws — guarantees the other email
   always gets its own independent attempt.
──────────────────────────────────────────── */
async function sendEmail(label, payload) {
  try {
    ...
    if (error) {
      // Resend returned a structured API error (wrong address, domain block, etc.)
      console.error(`❌ [${label}] Resend API error:`, JSON.stringify(error));
      return false;
    }

    console.log(`✅ [${label}] Sent successfully. Resend ID: ${data?.id}`);
    return true;

  } catch (err) {
    // Network-level or unexpected throw
...
router.post('/', async (req, res) => {
  try {
    ...
    /* ── Fire both emails independently ── */
    // Promise.allSettled guarantees BOTH always run — one failure cannot skip the other.
    const [adminResult, userResult] = await Promise.allSettled([

      sendEmail('ADMIN', {
        from:    'VELORA <onboarding@resend.dev>',
        to:      'veloradigital07@gmail.com',   // ✅ FIXED: was "to: to:" — duplicate key syntax error
    ...
    // Surface any delivery failures in Render logs without blocking the response
    const adminOk = adminResult.status === 'fulfilled' && adminResult.value === true;
    const userOk  = userResult.status  === 'fulfilled' && userResult.value  === true;

    if (!adminOk) console.warn('⚠️  Admin notification was NOT delivered — check logs above.');
    if (!userOk)  console.warn('⚠️  User auto-reply was NOT delivered — check logs above.');

    /* ── Always respond 200 once the contact is saved ── */
